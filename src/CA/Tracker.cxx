...
void TrackerTraits<false>::computeLayerTracklets(PrimaryVertexContext& primaryVertexContext)
{
	...
	const float3 &primaryVertexTmp = primaryVertexContext.getPrimaryVertex();
	std::cout<<"primary vertex: "<<primaryVertexTmp.x<<" "<<primaryVertexTmp.y<<" "<<primaryVertexTmp.z<<" "<<std::endl;
...
std::vector<std::vector<Road>> Tracker<IsGPU>::clustersToTracks(const Event& event)
{
  ...
  for (int iVertex { 0 }; iVertex < verticesNum; ++iVertex) {
    ...
    computeTracklets();
    for(int i=0;i<6;i++){
		iNumTrackletFound=mPrimaryVertexContext.getTracklets()[i].size();
		std::cout<<"Tracklet found for layer "<<i<<": "<<iNumTrackletFound<<std::endl;
		total+=iNumTrackletFound;
	}

	std::cout<<"Total tracklets found: "<<total<<std::endl;
/*    computeCells();
    findCellsNeighbours();
    findTracks();
